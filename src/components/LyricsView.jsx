    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-[#0d0d14]">
      ...
      <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <label className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 text-gray-300 rounded-lg text-xs cursor-pointer transition-colors border border-white/10">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-300 rounded-lg text-xs font-medium border border-indigo-500/30 transition-colors"
          >
      ...
      {isEditing ? (
        <div className="flex-1 p-6 flex flex-col gap-4">
          <p className="text-xs text-gray-400">
            Paste LRC timestamped lyrics (e.g. <span className="font-mono text-indigo-300">[00:12.50] Hello world</span>) or plain text lyrics below:
          </p>
          <textarea
            value={lyricsText}
            onChange={(e) => setLyricsText(e.target.value)}
            placeholder="[00:15.00] Enter lyrics line with timestamps here..."
            className="flex-1 bg-white/5 border border-white/10 rounded-xl p-4 text-xs font-mono text-gray-200 focus:outline-none focus:border-indigo-500 resize-none"
          />
            ...
            return (
              <p
                ...
                className={`cursor-pointer transition-all duration-300 select-none ${
                  isActive
                    ? 'text-2xl font-bold text-indigo-300 scale-105 drop-shadow-[0_0_20px_rgba(99,102,241,0.6)]'
                    : 'text-sm font-medium text-gray-500 hover:text-gray-300'
                }`}
              >
                {line.text}
              </p>
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-gray-500 text-xs p-8">
          <FileText className="w-12 h-12 mb-3 text-gray-600" />
          <p className="font-semibold text-gray-400">No lyrics loaded for this track</p>
